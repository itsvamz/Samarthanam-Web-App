...
const volunteerQuestions = [
  ...
  { field: 'password', question: 'Please enter your password:' },
  { field: 'confirmPassword', question: 'Please confirm your password:' },
...
const eventCreationQuestions = [
  ...
  { field: 'date', question: 'Please enter the event date :' },
];
...
const createSummary = (data: Record<string, string>, questions: { field: string; question: string }[]) => {
  ...
  questions.forEach(q => {
    ...
    // Mask password fields in summary
    let value = data[q.field] || "Not provided";
    if (q.field === 'password' || q.field === 'confirmPassword') {
      value = '********';
    }
...
const formatDate = (input: string): string | null => {
  ...
  // Check if we have 8 digits (YYYYMMDD)
  if (cleaned.length === 8) {
...
// Add email formatter helper function
...
const Chatbot: React.FC = (): React.ReactElement => {
  ...
  useEffect(() => {
    ...
    if (lastMessage.isBot) {
      if (lastMessage.type === 'button' && lastMessage.buttons) {
        const optionsText = lastMessage.buttons
          .map((btn, index) => `${index + 1}. ${btn.text}`)
          .join(', ');
        const fullText = `${lastMessage.text} Please choose an option: ${optionsText}`;
  ...
  const isAffirmativeResponse = (input: string): boolean => {
    const affirmativeWords = ['yes', 'yeah', 'yep', 'sure', 'okay', 'ok', 'yup', 'correct', 'affirmative', 'yes please', 'yes please'];
  ...
  const handleVoiceServiceChoice = (enableVoice: boolean) => {
    ...
    if (enableVoice) {
      startListening();
      addMessage("Voice assistance enabled!", true);
    } else {
      stopListening();
      addMessage("Going ahead without voice assistance!", true);
    }
  ...
  const handleVolunteerAnswer = () => {
    ...
    if (currentQuestion.field === 'contactNumber') {
      const formatted = formatPhoneNumber(answer);
      if (formatted) {
      ...
      formattedAnswer = formatEmail(answer);
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(formattedAnswer)) {
        isValid = false;
        errorMessage = 'Please enter a valid email address.';
      }
    } else if (currentQuestion.field === 'startDate' || currentQuestion.field === 'endDate') {
      const formatted = formatDate(answer);
      if (formatted) {
        formattedAnswer = formatted;
      } else {
        isValid = false;
    ...
    } else if (currentQuestion.field === 'confirmPassword') {
      if (answer !== volunteerData['password']) {
        isValid = false;
        errorMessage = 'Passwords do not match. Please enter your password again:';
        setVolunteerData(prev => ({
          ...prev,
          password: '',
          confirmPassword: ''
        }));
        const passwordIndex = volunteerQuestions.findIndex(q => q.field === 'password');
        setVolunteerQuestionIndex(passwordIndex);
      }
    }
    ...
    // Add the formatted answer to messages
    addMessage(formattedAnswer, false);

    setVolunteerData((prev) => ({
      ...prev,
      [currentQuestion.field]: formattedAnswer,
    }));
    ...
    if (nextIndex < volunteerQuestions.length) {
      ...
      const finalData = { ...volunteerData, [currentQuestion.field]: formattedAnswer };
  ...
  const handleParticipantAnswer = () => {
    ...
    if (currentQuestion.field === 'contactNumber') {
      ...
      if (answer !== participantData['password']) {
        isValid = false;
        errorMessage = 'Passwords do not match. Please enter your password again:';
        ...
        const passwordIndex = participantQuestions.findIndex(q => q.field === 'password');
        setParticipantQuestionIndex(passwordIndex);
      }
    }
    ...
    if (nextIndex < participantQuestions.length) {
      ...
      const finalData = { ...participantData, [currentQuestion.field]: formattedAnswer };
      const summary = createSummary(finalData, participantQuestions);
      addMessage(summary + "\nIs this correct? (Yes/No)", true);
      setChatState('participantConfirm');
    }
  };
  ...
  function isValidNumberWord(key: string): key is keyof typeof numberWords {
    return numberWords.hasOwnProperty(key);
  }
  
  const getOptionNumber = (input: string): number | null => {
    const trimmed = input.trim().toLowerCase();
    if (/^\d+$/.test(trimmed)) {
      return parseInt(trimmed, 10);
    } else if (isValidNumberWord(trimmed)) {
  ...
  const handleSendMessage = () => {
    if (inputMessage.trim() !== '') {
      if (chatState === 'welcome') {
        const option = getOptionNumber(inputMessage);
        if (option !== null && option >= 1 && option <= 5) {
          addMessage(inputMessage, false); // Display user's input
          ...
          handleMainMenuClick(states[option - 1]);
        } else {
          addMessage(inputMessage, false); // Display user's input
          addMessage('Invalid option. Please choose a number between 1 and 5.', true);
        }
      } else if (chatState === 'volunteerConfirm') {
          ...
          <Box sx={{ mt: 2, display: 'flex', gap: 2 }}>
            <Button 
              ...
              onClick={() => {
                ...
                addMessage("Are you sure you want to submit? Type 'yes' to confirm or 'no' to cancel.", true);
